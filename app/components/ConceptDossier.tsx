...
function generationFooterParts(data: DossierData, requestMode: 'live' | 'queued') {
  const parts = [
    data.profile ?? requestMode,
    data.synthesis_variant === 'spanify_precontext' ? 'precontext linked brief' : 'dossier',
  ];
  if (data.generator) parts.push(`generated by ${data.generator}`);
  if (data.model && data.model !== 'no-ai') parts.push(data.model);
  if (data.generator === 'local') parts.push('demo mode');
  return parts;
}
      ...
      <div style={{ padding: '22px 28px 36px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {!loading && data?.paragraph && (
          <section>
            ...
            />
            {branchContextName && (
              <div style={{ marginBottom: 12, fontSize: '0.72rem', color: '#777', lineHeight: 1.5 }}>
                Opened <strong style={{ color: '#bbb' }}>{targetName}</strong> from{' '}
                <strong style={{ color: '#bbb' }}>{branchContextName}</strong>. Your branch trail is preserved.
              </div>
            )}
            <ParagraphView
            ...
            {data.generator && (
              <div style={{ marginTop: 14, fontSize: '0.55rem', color: '#333', letterSpacing: '0.16em', textTransform: 'uppercase' }}>
                {generationFooterParts(data, requestMode).join(' · ')}
              </div>
            )}
          </section>
        )}
...
function PendingBanner({
  ...
  const isDemoMode = error?.code === 'local-reader-mode' || error?.code === 'demo-mode-cache-miss';
  ...
  const body = isAuth ? (
    ...
    <>
      This concept was not pregenerated for Demo Mode. Some demo links are intentionally here to
      show where the full version begins. Use API config with your own project materials to generate
      a hypertext dossier for <strong style={{ color: '#eee' }}>{name}</strong>.
    </>
  ) : (
    <>
      {mode === 'queued' ? (
      ...
      <div style={{ color: '#777', fontSize: '0.82rem', lineHeight: 1.6 }}>
        {mode === 'queued' ? (
          ...
          <>
            ...
            . Bird Brain is reusing the brief and only adding clickable spans. The result is cached afterwards.
          </>
        )}
      </div>
      <div
